...
import { sizeOf, toChunks, type GTFSSource } from "./Source.js";
...
describe("toChunks", () => {
  ...
  it("reads a Uint8Array", async () => {
    expect(await collect(BYTES)).to.deep.equal([1, 2, 3, 4, 5]);
  });

  it("reads a view onto part of a buffer without reading the rest of it", async () => {
    const view = new Uint8Array(BYTES.buffer, 1, 3);

    expect(await collect(view)).to.deep.equal([2, 3, 4]);
  });

  it("reads an ArrayBuffer", async () => {
    expect(await collect(BYTES.buffer as ArrayBuffer)).to.deep.equal([1, 2, 3, 4, 5]);
  });

  it("reads a Blob", async () => {
    expect(await collect(new Blob([BYTES]))).to.deep.equal([1, 2, 3, 4, 5]);
  });

  it("reads a Response", async () => {
    expect(await collect(new Response(BYTES))).to.deep.equal([1, 2, 3, 4, 5]);
  });

  it("reads a ReadableStream", async () => {
    expect(await collect(new Response(BYTES).body as ReadableStream<Uint8Array>)).to.deep.equal([1, 2, 3, 4, 5]);
  });
  ...
  it("reads a Node readable stream", async () => {
    ...
    expect(await collect(stream)).to.deep.equal([1, 2, 3, 4, 5]);
  });
  ...
  it("reads a stream opened with an encoding, which gives strings", async () => {
    ...
    expect(await collect(stream as unknown as GTFSSource)).to.deep.equal([97, 98, 99]);
  });

  it("reads a plain iterable of chunks", async () => {
    expect(await collect([new Uint8Array([1, 2]), new Uint8Array([3, 4, 5])])).to.deep.equal([1, 2, 3, 4, 5]);
  });

  it("says so rather than failing obscurely when given something it cannot read", () => {
    expect(() => toChunks(42 as unknown as GTFSSource)).to.throw(/Cannot read a GTFS feed/);
  });
...
describe("sizeOf", () => {
  ...
  it("knows the size of bytes", () => {
    expect(sizeOf(BYTES)).to.equal(5);
    expect(sizeOf(BYTES.buffer as ArrayBuffer)).to.equal(5);
  });

  it("knows the size of a Blob", () => {
    expect(sizeOf(new Blob([BYTES]))).to.equal(5);
  });

  it("takes the size of a Response from its content length", () => {
    const response = new Response(BYTES, { headers: { "content-length": "5" } });

    expect(sizeOf(response)).to.equal(5);
  });

  it("does not guess the size of a stream", () => {
    expect(sizeOf(Readable.from([BYTES]))).to.equal(undefined);
  });
