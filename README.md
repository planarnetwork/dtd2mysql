...
| [`transfer-patterns.gz`](https://github.com/planarnetwork/gb-transit/releases/latest/download/transfer-patterns.gz) | the stations a journey can change at, for a transfer pattern journey planner |
...
`transfer-patterns.gz` is not a feed but a companion to the first one: 57 million routes through the
