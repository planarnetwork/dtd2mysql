...
/** How many rows a page shows. Enough to see a pattern, few enough to be a table. */
...
export function fileTable(page: Page, route: Extract<Route, {view: "file"}>): string {
  ...
  const head = columns.map(column => {
    ...
    return `<th scope="col"><a href="${to}" class="x-sort">${escape(column)}${marker}</a>`
      + `<input class="x-filter" type="text" name="${escape(column)}" `
      + `value="${escape(route.filters[column] ?? "")}" `
      + `aria-label="Filter ${escape(column)}" placeholder="filter"></th>`;
  }).join("");

  const body = page.rows.map(({index, values}) => `<tr>
      <td class="x-rownum"><a href="${format({...route, page: pageOf(index, page)})}#row-${index}"
  ...
  return `
    <h2 class="h2 h2--sm" tabindex="-1" data-heading>${escape(page.file)}</h2>
    <p class="x-lede" aria-live="polite">
      ${page.matched === page.total
        ? `${number(page.total)} rows.`
        : `${number(page.matched)} of ${number(page.total)} rows match.`}
      ${page.notHeld.length > 0
        ? `<span class="x-note">${page.notHeld.map(escape).join(" and ")} `
          + `${page.notHeld.length === 1 ? "is" : "are"} read from the zip on demand and not shown here.</span>`
        : ""}
    </p>
    ${page.sortRefused === undefined ? "" : `<p class="x-warn">${escape(page.sortRefused)}</p>`}
    <form class="x-tools" data-filters>
      <div class="x-scroll">
        <table class="x-table x-table--data">
          <thead><tr><th scope="col" class="x-rownum">row</th>${head}</tr></thead>
          <tbody>${body}</tbody>
        </table>
      </div>
    </form>
    ${page.rows.length === 0 ? "<p class=\"x-empty\">Nothing matches those filters.</p>" : ""}
    ${pager(route, page, pages)}
    <p class="x-tools">
      <button class="x-btn" data-export="csv">Export the ${number(page.matched)} matching rows as CSV</button>
      <button class="x-btn" data-export="json">as JSON</button>
    </p>`;
}
...
function pager(route: Extract<Route, {view: "file"}>, page: Page, pages: number): string {
  ...
  const step = (label: string, index: number, enabled: boolean) => enabled
    ? `<a class="x-btn" href="${to(index)}">${label}</a>`
    : `<span class="x-btn x-btn--off" aria-disabled="true">${label}</span>`;

  return `<nav class="x-pager" aria-label="Pages of ${escape(page.file)}">
    ${step("&larr; first", 0, at > 0)}
    ${step("previous", at - 1, at > 0)}
    <span class="x-pager__at">page ${number(at + 1)} of ${number(pages)}</span>
    ${step("next", at + 1, at < pages - 1)}
    ${step("last &rarr;", pages - 1, at < pages - 1)}
    <label class="x-jump">go to row
...
function pageOf(row: number, page: Page): number {
  return Math.floor((page.offset + page.rows.findIndex(r => r.index === row)) / PAGE_SIZE);
}
