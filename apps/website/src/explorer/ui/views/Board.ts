...
export function boardView(detail: BoardDetail): string {
  const heading = `
    ...
    <nav class="tools" aria-label="Other days">
      <a class="btn2" href="${format({view: "board", id: detail.stopId,
  ...
  const rows = detail.departures.map(departure => `<tr${departure.pickup === 1
    ? " class=\"row--quiet\"" : ""}>
      <td class="board__time">${escape(departure.time)}${departure.previousDay
        ? "<span class=\"board__over\" title=\"runs on the previous day's service, after midnight\">+</span>"
        : ""}</td>
      <td class="board__to">${escape(departure.destination ?? departure.headsign ?? "")}</td>
      <td class="board__plat">${escape(departure.platform ?? "")}</td>
      <td class="board__id">
        <a href="${format({view: "trip", id: departure.tripId})}">${escape(departure.shortName
          ?? departure.tripId)}</a>
        ${departure.pickup === 1 ? "<span class=\"board__note\">nobody boards</span>" : ""}
      </td>
    </tr>`).join("");
  ...
  return `${heading}
    ...
    ${overnight === 0 ? "" : `<p class="note">
      ${number(overnight)} of these are marked <span class="board__over">+</span>: they are written
