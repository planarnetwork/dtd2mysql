...
same trips. See [the passing points feed](#the-passing-points-feed). Drawing a train on a map needs
neither: both feeds carry the same [shapes](#shapes).
...
stations the train touches, whether it calls there or runs through. **Both** feeds carry it, and
they carry the identical file — the line is drawn from every timing point regardless of which of
them the feed publishes as a call, which is what the nightly checks before releasing.
...
The standard feed drops them from `stop_times.txt` — though not from the shapes, which are drawn
through them in both feeds. `gtfs-passing-points.zip` keeps them as calls, with `pickup_type` and
...
Everything else is identical. The two feeds hold the same trips, routes, calendars and shapes, and
the nightly stops rather than publishing if the two disagree about how many trips there are or draw
different lines.
