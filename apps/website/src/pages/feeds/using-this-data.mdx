...
Patterns are found for one day at a time, and the published file holds those of the next Tuesday,
Friday, Saturday and Sunday together: what changes between days is the shape of the service rather
than the geography, and four consecutive days would be four Tuesdays.

It is built after the feed it describes is published, so a release that does not carry it is a night
the scan did not finish rather than a feed with something wrong. [raptor](https://github.com/planarnetwork/raptor)
finds the patterns and reads them back; `readPatterns` is the function.
